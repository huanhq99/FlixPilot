...
<p align="center">
  ...
  <sub>Go API + Caddy/Vite 前端的双容器版本，把求片、追剧、缺集、播放风控、会员积分、通知模板、Telegram Bot、AI Agent 和 115 联动收进同一个工作台。</sub>
</p>
...
<p align="center">
  ...
  <a href="https://hub.docker.com/r/huanhq99/flixpilot-go"><img src="https://img.shields.io/docker/pulls/huanhq99/flixpilot-go?style=for-the-badge&logo=docker&color=0b84a5&label=Pulls" alt="docker pulls"></a>
  <a href="https://hub.docker.com/r/huanhq99/flixpilot-go"><img src="https://img.shields.io/docker/v/huanhq99/flixpilot-go?style=for-the-badge&logo=docker&label=Version&color=f59e0b" alt="version"></a>
...
当前仓库是 **FlixPilot-Go**：前端由 Vite 构建成静态文件，生产环境用 `huanhq99/flixpilot-go` 镜像里的 Caddy 对外提供 `3005`；后端能力由独立的 `huanhq99/flixpilot-api` Go 服务提供，容器内监听 `8080`，并额外负责 Emby 反代网关端口段。
...
升级前建议先备份 `data/` 目录。最近更新见 [v2.4.4 版本日志](docs/changelog/v2.4.4.md)。
...
推荐使用 Docker Compose。镜像：[`huanhq99/flixpilot-go`](https://hub.docker.com/r/huanhq99/flixpilot-go) + [`huanhq99/flixpilot-api`](https://hub.docker.com/r/huanhq99/flixpilot-api)，支持 `linux/amd64` 和 `linux/arm64`。默认使用 `latest`

### 服务架构

| 服务 | 镜像 | 作用 | 默认端口 |
|------|------|------|----------|
| `flixpilot` | `huanhq99/flixpilot-go` | Caddy 托管 Vite 静态前端，并把 `/api/*` 反代到 Go API | `3005` |
| `flixpilot-api` | `huanhq99/flixpilot-api` | Go 后端，处理认证、数据、Webhook、Telegram、Emby/MoviePilot/TMDB 等集成 | 容器内 `8080` |
| `flixpilot-api` 网关 | `huanhq99/flixpilot-api` | 可选的 Emby 反代网关，用于接管播放入口和风控 | `8091-8095` |
...
| Redis | 默认 compose 已自带，作为缓存/限流使用；自定义部署时可接外部 Redis |
...
| 部署 | Docker Compose 双镜像：Caddy 前端 `3005` + Go API `8080` + 可选 Emby 网关 |
| 包管理 | pnpm |
| 测试 | Vitest + Go test |
...
├── scripts/                 # 工具脚本
├── docker-compose.yml
├── Caddyfile                # 前端镜像的 Caddy 配置，反代 /api 到 Go API
├── Dockerfile               # 前端静态站点镜像
├── backend/Dockerfile       # Go API 镜像
└── package.json
```
...
开发服务默认运行在 `http://localhost:3005`。生产运行时不跑 Vite/Node 开发服务，前端由 Caddy 静态托管，API 由 Go 容器提供。
