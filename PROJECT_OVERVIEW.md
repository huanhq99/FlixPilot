# FlixPilot 项目总览（2026-03-10 `v2.2.5.025`）
助.
## 项目定位
...
src/
  ...
  @layouts/                  # 页面布局
  @menu/                     # 菜单配置与组件（支持管理员子菜单动态权限）
  components/                # 通用组件
  lib/                       # 业务逻辑库（Emby、GoEdge、邮件、Redis、下载器驱动、文件名解析器等）
  services/                  # 外部服务接口
  types/                     # 类型定义
  utils/                     # 工具函数
  views/                     # 主要页面视图
...
### 2. 媒体维护与缺集管理
- **高并发扫码引擎**：服务端 30+ 并发线程，支持“总集数差值”秒杀跳过优化，数百部剧扫描耗时 < 20s。
- **智能化缺集审计**：自动比对 Emby 本地库存与 TMDB 线上数据，精准定位缺失季度与集数。
- **追剧日历**：实时展示剧集更新，结合本地库存显示“已入库/未入库”状态。
- **下载器截胡**：支持 qBittorrent/Transmission，拦截 MP 侧发送的种子并精确切除非必要集数。
...
### 6. 媒体运维中心 (Media Ops)
- 二级菜单整合：缺集管理、库品质分析（分辨率/编码/大小）、求片处理。

### 7. 其他特性
- **性能优化**：具备动态并发流控与内存回收机制，支持低配 NAS 稳定运行。
- 支持多主题、暗黑模式、多语言。
- 支持 Docker 一键部署。
- 支持自定义菜单、首页模块。
- 第三方对接：MoviePilot、TMDB、Redis、Telegram Bot 等。
